...
function Client(options) {
  ...
  this.s3PendDel = new Pend();
  this.s3PendDel.max = options.maxAsyncS3Del || 1;
  ...
  this.ignoreInv = options.ignore ? options.ignore.charAt(0) === '!' : false;
  this.ignore = this.ignoreInv ?  options.ignore.substr(1) : options.ignore;
  this.noMD5 = !options.MD5;
}
...
Client.prototype.deleteObjects = function(s3Params) {
  ...
  function uploadSlice(slice) {
    ...
    function tryDeletingObjects(cb) {
      self.s3PendDel.go(function(pendCb) {
...
Client.prototype.uploadFile = function(params) {
  ...
  uploader.end = function() {
    console.error("\ruploaded: " + s3Params.Key + "    \n");
  }
  ...
  function openFile() {
    fs.open(localFile, 'r', function(err, fd) {
      if (err) return handleError(err, 'open');
      ...
      fs.fstat(fd, function(err, stat) {
        if (err) return handleError(err, 'fstat');
        localFileStat = stat;
  ...
  function startPuttingObject() {
    ...
    function onPutObjectDone(err, data) {
      if (fatalError) return;
      if (err) return handleError(err, 'putObject');
      if (localFileSlicer) {
        localFileSlicer.unref();
        localFileSlicer = null;
      }
  ...
  function startMultipartUpload(multipartUploadSize) {
    doWithRetry(tryCreateMultipartUpload, self.s3RetryCount, self.s3RetryDelay, function(err, data) {
      if (fatalError) return;
      if (err) return handleError(err, 'multipartUpload');
  ...
  function queueAllParts(uploadId, multipartUploadSize) {
    ...
    pend.wait(function(err) {
      if (fatalError) return;
      if (err) return handleError(err, 'wait');
  ...
  function makeUploadPartFn(start, end, part, uploadId) {
    return function(cb) {
      doWithRetry(tryUploadPart, self.s3RetryCount, self.s3RetryDelay, function(err, data) {
        if (fatalError) return;
        if (err) return handleError(err, 'uploadPart');
    ...
    function tryUploadPart(cb) {
      ...
      self.s3Pend.go(function(pendCb) {
        ...
        inStream.on('error', function(err) {
          ...
          handleError(err, 'createReadStream');
        });
  ...
  function completeMultipartUpload() {
    ...
    doWithRetry(tryCompleteMultipartUpload, self.s3RetryCount, self.s3RetryDelay, function(err, data) {
      if (fatalError) return;
      if (err) return handleError(err, 'tryCompleteMultipartUpload');
      uploader.emit('end', data);
    });
  }
  ...
  function tryPuttingObject(cb) {
    self.s3Pend.go(function(pendCb) {
      ...
      inStream.on('error', function(err) { handleError(err, 'tryPuttingObject'); });
      var pend = new Pend();
...
Client.prototype.downloadFile = function(params) {
  ...
  function doTheDownload(cb) {
    ...
    request.on('httpHeaders', function(statusCode, headers, resp) {
      if (statusCode >= 300) {
        handleError(new Error("http status code " + statusCode), 'doTheDownload');
        return;
      }
      if (headers['content-length'] == undefined) {
        var outStream = fs.createWriteStream(localFile);
        outStream.on('error', function(err) {handleError(err, 'doTheDownload.outStream');});
        ...
        httpStream.on('error', function(err) {handleError(err, 'doTheDownload.httpStream');});
        outStream.on('error', function(err) {handleError(err, 'doTheDownload.outStream');});

        hashCheckPend.go(function(cb) {
          multipartETag.on('end', function() {
            if (multipartETag.bytes !== contentLength) {
              handleError(new Error("Downloaded size does not match Content-Length"), 'doTheDownload');
              return;
            }
            if (eTagCount === 1 && !multipartETag.anyMatch(eTag)) {
              handleError(new Error("ETag does not match MD5 checksum"), 'doTheDownload');
              return;
            }
            cb();
          });
...
Client.prototype.listObjects = function(params) {
  ...
  function findAllS3Objects(marker, prefix, cb) {
    ...
    doWithRetry(listObjects, self.s3RetryCount, self.s3RetryDelay, function(err, data) {
      ...
      if (self.ignore) {
        data.Contents.forEach(function(item, index, obj) {
          if (item.Key.endsWith(self.ignore) !== self.ignoreInv)
            obj.splice(index, 1);
        });
      }
...
Client.prototype.downloadBuffer = function(s3Params) {
  ...
  function doTheDownload(cb) {
    ...
    request.on('httpHeaders', function(statusCode, headers, resp) {
      ...
      httpStream.on('error', function(err) {handleError(err, 'doTheDownload.outStream');});
      outStream.on('error', function(err) {handleError(err, 'doTheDownload.outStream');});
...
function syncDir(self, params, directionIsToS3) {
  ...
  var listObjectsParams = {
    ...
    s3Params: {
      ...
      MaxKeys: null,
      Prefix: prefix,
      RequestPayer: params.s3Params.RequestPayer,
    },
  };
  ...
  function flushDeletes() {
    ...
    var deleter = self.deleteObjects(params);
    deleter.on('error', function(err) {handleError(err, 'flushDeletes');});
    deleter.on('end', function() {
      if (fatalError) return;
  ...
  function checkDoMoreWork() {
    ...
    // sync needs the two list sorted
    if (!ee.doneMd5 || !ee.doneFindingObjects) return;
    ...
    if (localFileStat && !self.noMD5 && !localFileStat.multipartETag) return;
    ...
    if (!localFileStat && !s3Object) {
      ...
      process.stderr.write("Done " + localFileCursor + " files, " + s3ObjectCursor + " objects.\n");
    ...
    if (directionIsToS3) {
      if (!localFileStat) {
        ...
        // process.stderr.write("uploading " + localFileStat.s3Path + " size: " + localFileStat.size + "\n");
        uploadLocalFile();
      } else if (localFileStat.s3Path < s3Object.key) {
        // process.stderr.write("uploading " + localFileStat.s3Path + " size: " + localFileStat.size + "(s3 cursor: " + s3Object.key + ")\n");
        uploadLocalFile();
      } else if (localFileStat.s3Path > s3Object.key) {
        deleteS3Object();
      } else if (!self.noMD5 && !compareMultipartETag(s3Object.ETag, localFileStat.multipartETag)){
        // both file cursor and s3 cursor should increment
        s3ObjectCursor += 1;
        uploadLocalFile();
      } else if (self.noMD5 && parseInt(s3Object.Size) != localFileStat.size){
        // process.stderr.write("uploading " + localFileStat.s3Path + " s3 size: " + s3Object.Size + " local size: " + localFileStat.size + "\n");
      ...
      if (!localFileStat) {
        //process.stderr.write("downloading " + s3Object.key + " s3 size: " + s3Object.Size + "\n")
        downloadS3Object();
      } else if (!s3Object) {
        //process.stderr.write("deleting " + localFileStat.s3Path + "\n")
        deleteLocalFile();
      } else if (localFileStat.s3Path < s3Object.key) {
        //process.stderr.write("deleting " + localFileStat.s3Path + " (s3.key = " + s3Object.key + ")\n")
        deleteLocalFile();
      } else if (localFileStat.s3Path > s3Object.key) {
        //process.stderr.write("downloading " + s3Object.key + " s3 size: " + s3Object.Size + " (local: " + localFileStat.s3Path + ")\n")
        downloadS3Object();
      } else if (!self.noMD5 && !compareMultipartETag(s3Object.ETag, localFileStat.multipartETag)){
        // both file cursor and s3 cursor should increment
        localFileCursor += 1;
        ...
        process.stderr.write("updating " + localFileStat.s3Path + " s3 size: " + s3Object.Size + " local size: " + localFileStat.size + "\n")
        localFileCursor += 1;
        downloadS3Object();
      } else {
        //process.stderr.write("skipping " + s3Object.key + " s3 size: " + s3Object.Size + " (local: " + localFileStat.s3Path + ")\n")
        skipThisOne();
      }
    }
    ...
    function deleteLocalDir() {
      ...
      rimraf(fullPath, function(err) {
        if (fatalError) return;
        if (err && err.code !== 'ENOENT') return handleError(err, 'deleteLocalDir');
        ee.deleteAmount += 1;
        ee.emit('progress');
        checkDoMoreWork();
      });
    ...
    function deleteLocalFile() {
      ...
      fs.unlink(fullPath, function(err) {
        if (fatalError) return;
        if (err && err.code !== 'ENOENT') return handleError(err, 'deleteLocalFile');
        ee.deleteAmount += 1;
        ee.emit('progress');
        checkDoMoreWork();
      });
    ...
    function downloadS3Object() {
      ...
      function haveS3Params(err, s3Params) {
        if (fatalError) return;
        if (err) return handleError(err, 'getS3Params');
      ...
      function startDownload() {
        ...
        downloader.on('error', function(err) {handleError(err, 'downloadS3Object');});
    ...
    function uploadLocalFile() {
      ...
      function haveS3Params(err, s3Params) {
        if (fatalError) return;
        if (err) return handleError(err, 'uploadLocalFile');
      ...
      function startUpload() {
        ...
        uploader.on('error', function(err) {handleError(err, 'uploadLocalFile '+fullKey);});
        ...
        uploader.on('end', function() {
          console.error("\ruploaded " + fullKey + "    \n");
          ee.activeTransfers--;
  ...
  function findAllS3Objects() {
    var finder = self.listObjects(listObjectsParams);
    finder.on('error', function(err) {
      handleError(err, 'findAllS3Objects');
    });
    finder.on('data', function(data) {
      ...
      data.Contents.forEach(function(object) {
        ...
        if (!self.ignore || object.key.endsWith(self.ignore) == self.ignoreInv)
    ...
    finder.on('end', function() {
      if (fatalError) return;
      ee.emit('progress');
      process.stderr.write("\nfindAllS3Objects: " + allS3Objects.length + ", sorting ...\n");
      ...
      ee.emit('progress');
      process.stderr.write("allS3Objects sorted\n");
  ...
  function startFindAllFiles() {
    findAllFiles(function(err) {
      if (fatalError) return;
      if (err) return handleError(err, 'findAllFiles');
      ...
      ee.emit('progress');
      process.stderr.write("\nfindAllFiles: " + allLocalFiles.length + "\n");
  ...
  function startComputingMd5Sums() {
    ...
    function computeOne() {
      ...
      inStream.on('error', function(err) {handleError(err, 'startComputingMd5Sums');});
      var prevBytes = 0;
      multipartETag.on('progress', function() {
        var delta = multipartETag.bytes - prevBytes;
        prevBytes = multipartETag.bytes;
  ...
  function findAllFiles(cb) {
    ...
    walker.on('directory', function(dir, stat, stop, linkPath) {
      //process.stderr.write("subdir: " + dir + (fatalError ? " fatal\n" : "\n"));
    ...
    walker.on('file', function(file, stat, linkPath) {
      //process.stderr.write("file: " + file + (fatalError ? " fatal\n" : "\n"));
      if (fatalError) return walker.stop();
      if (!self.ignore || file.endsWith(self.ignore) == self.ignoreInv) {
    ...
    fn(function(err, result) {
      if (err) {
        if (err.retryable === false) {
          err.message += " (non-retryable)";
          cb(err);
        } else {
          tryIndex += 1;
          if (tryIndex >= tryCount) {
            err.message += " (retries: " + tryCount + ")";
            cb(err);
          } else {
...
function getPublicUrl(bucket, key, bucketLocation, endpoint) {
  ...
  var hostnamePrefix = nonStandardBucketLocation ? bucketLocation : "s3";
  var parts = {
    protocol: "https:",
