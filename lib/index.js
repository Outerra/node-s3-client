...
function syncDir(self, params, directionIsToS3) {
  ...
  function findAllS3Objects() {
    ...
    finder.on('end', function() {
      ...
      //sort to have desc files at the end
      allS3Objects.sort(function(a, b) {
        var adesc = a.key.endsWith(".desc");
        var bdesc = b.key.endsWith(".desc");
        if (adesc != bdesc)
            return adesc ? 1 : -1;
  ...
  function startFindAllFiles() {
    findAllFiles(function(err) {
      ...
      allLocalFiles.sort(function(a, b) {
        //order desc files at the end
        var adesc = a.s3Path.endsWith(".desc");
        var bdesc = b.s3Path.endsWith(".desc");
        if (adesc != bdesc)
            return adesc ? 1 : -1;
...
/*
      process.stderr.write("sort order: \n");
      for (var s=0; s < allLocalFiles.length; ++s) {
        process.stderr.write(allLocalFiles[s].s3Path + '\n');
      }
*/
